...
public class Prayer {
    ...
    private static double getTimeZone1() {
    ...
    private ArrayList<String> getDatePrayerTimes(int year, int month, int day,
                                                 double latitude, double longitude, double tZone,Context ctx) {
        ...
        return computeDayTimes(ctx);
    }

    // return prayer times for a given date
    public ArrayList<String> getPrayerTimes(Calendar date, double latitude,
                                             double longitude, double tZone,Context cxt) {

        int year = date.get(Calendar.YEAR);
        int month = date.get(Calendar.MONTH);
        int day = date.get(Calendar.DATE);

        return getDatePrayerTimes(year, month+1, day, latitude, longitude, tZone,cxt);
    }
    ...
    public String floatToTime12(double time, boolean noSuffix, Context ctx) {
    ...
    public String floatToTime12NS(double time,Context ctx) {
        return floatToTime12(time, true,ctx);
    }
    ...
    // compute prayer times at given julian date
    private ArrayList<String> computeDayTimes(Context ctx) {
        ...
        return adjustTimesFormat(times,ctx);
    }
    ...
    private ArrayList<String> adjustTimesFormat(double[] times,Context ctx) {
        ...
        for (int i = 0; i < 7; i++) {
            if (this.getTimeFormat() == this.getTime12()) {
                result.add(floatToTime12(times[i], false,ctx));
            } else if (this.getTimeFormat() == this.getTime12NS()) {
                result.add(floatToTime12(times[i], true,ctx));
            } else {
...
/*
    public static List<ItemSalat> main(double lal, double lng) {


        List<ItemSalat> salatList = new ArrayList<>();

        double latitude = -35.656565;
        double longitude = 125.000;
        double timezone = getTimeZone1();
        ...
        for (int i = 0; i < prayerTimes.size(); i++) {
           // System.out.println(prayerNames.get(i) + " - " + prayerTimes.get(i));
            salatList.add(new ItemSalat(prayerTimes.get(i),prayerNames.get(i)));
        }
