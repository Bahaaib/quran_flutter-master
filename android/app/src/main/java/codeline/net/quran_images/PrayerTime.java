...
public class PrayerTime {
    ...
    public static List<String> getSalatTimes(Context context,double lat, double lng,Calendar calendar,int format) {
        return getPrayerTimes(context, lat, lng,calendar,format);
    }

    public static List<String> getPrayerTimes(Context context, double lat, double lng,Calendar calendar,int format) {
       // AppSettings settings = AppSettings.getInstance(context);
        ...
        if(format == -1)
            ...
            prayers.setTimeFormat(DataSalatCalu.getTimeFormat(context));


        prayers.setCalcMethod(MAKKAH);
        prayers.setAsrJuristic(SHAFII);
        prayers.setAdjustHighLats(DataSalatCalu.getAngleCalul(context));
